...
def race_traj(trajs,
    ...
    while True:
        ...
        # race
        inst_ids = set(range(cost_data_incs.shape[0]))
        MIN_INSTS = 5
        used_insts = np.random.choice(list(inst_ids), size=MIN_INSTS).tolist()
        active_confs = list(range(len(incs_avail)))
        inst_ids -= set(used_insts)
        ...
        while inst_ids:
            ...
            if not first:
                next_inst = np.random.choice(list(inst_ids))
                used_insts.append(next_inst)
                inst_ids -= set([next_inst])
            first = False
            
            #print(cost_data_incs.shape,used_insts,active_confs)
            race_cost = cost_data_incs[used_insts,:][:,active_confs]
            best_idx = np.argmin(np.mean(race_cost,axis=0))

            new_active_configs = []
            for id_, inc_id in enumerate(active_confs):
                x = race_cost[:,best_idx]
                y = race_cost[:,id_]
                x_better = perm_test(x,y)
                #print(x_better, np.mean(x), np.mean(y)) 
                if not x_better:
                    new_active_configs.append(inc_id)
                    
            if len(new_active_configs) == 1:
                break
            
            active_confs = new_active_configs

        if len(new_active_configs) == 1:
            inc = incs[new_active_configs[0]]
        else:
            costs = cost_data_incs[:,new_active_configs]
            avg_cost = np.mean(costs)
            best_idx = np.argmin(avg_cost)
            inc = incs[new_active_configs[best_idx]]
